...
class _HomePageState extends State<HomePage> {
  ...
  List<TodoModel> _listaCompleta = []; // Para mantener una copia local de la lista completa de tareas
  ...
  void _sincronizarConNativo(List<TodoModel> tareas) {
    ...
    print("Sincronización completa: ${tareas.length} items de API vs $totalEnC en C++");
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Sincronizado con C++: $totalEnC registros validados.')),
      );
    });
  }
  ...
  void _testBuscar(int id) {
    bool existe = _nativeService.buscar(id); // Debes añadir esta función en el NativeService
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('ID $id ${existe ? "encontrado en C++" : "no encontrado"}'),
              ...
              // Filtro para  la vista para mostrar solo esa tarea
                setState(() {
                  _futureTodos = Future.value(
                    _listaCompleta.where((tarea) => tarea.idTarea == id).toList() // usar la lista completa para filtrar y mostrar solo la tarea buscada, sin perder la referencia original de todas las tareas.
                  );
                });
            } else {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("No encontrada en C++")));
            }
          },
...
Future<void> _reestablecerTareas() async {
  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Restaurando datos...")));
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  leading: CircleAvatar(
                    ...
                    child: Text('${tarea.idTarea}', style: const TextStyle(fontWeight: FontWeight.bold)),
                  ),
                  ...
                  title: Row(
                    children: [
                      Expanded(
                        child: Text(
                          tarea.titulo,
                          maxLines: 1, // Evita que se desarme el diseño si el título es muy largo
                          overflow: TextOverflow.ellipsis, // Si no cabe, le pone tres puntitos (...)
                        ),
                      ),
                    ],
                  ),
                  ...
                  trailing: SizedBox(
                    width: 140, // Le damos un ancho fijo al contenedor de botones para que Flutter sepa cuánto espacio reservar
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                        children: [
                          ...
                          IconButton(
                            constraints: const BoxConstraints(), // Hace el botón más compacto
